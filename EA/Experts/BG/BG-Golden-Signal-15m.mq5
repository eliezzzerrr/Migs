//+------------------------------------------------------------------+
//|                                      BG-Golden-Signal-15m.mq5      |
//|                                            Copyright 2026, BG      |
//|                                                                    |
//|  PINE-PARITY BUILD — mirrors BG-Golden-Signal-15m.pine (v6).       |
...
#property description "BG Golden 15m v0.9.0 — Pine-parity build matching BG-Golden-Signal-15m.pine."
...
input bool   InpAsiaDowMon        = true;       // Asia day-of-week gate — Pine i_asia_dow_* (GMT+8). 15m = Mon/Tue/Fri
...
input bool   InpUseLondon         = true;       // London ENABLED — 15m trades Tue-Fri
input string InpLondonSession     = "14:00-17:00";
input bool   InpLondonDowMon      = false;      // London day-of-week gate — Pine i_london_dow_* (GMT+8). 15m = Tue-Fri
...
input long   InpMagic             = 20261504;   // distinct from 5m EA (20260522) so both coexist on one account
...
input string InpJournalDir        = "BG\\journal-15m";
...
// Per-session day-of-week gate for Asia (Pine i_asia_dow_*). 15m validated
// Asia set = Mon/Tue/Fri. Weekday read in the session TZ (GMT+8 / Manila).
...
void DrawPanel()
{
   ...
   EnsureLabel("hdr", 0, y, "BG GOLDEN 15m v" + BG_VERSION + " (Pine parity)", InpPanelHeaderClr); y += line_h;
...
int OnInit(void)
{
   MLog("INIT", StringFormat("BG-Golden-15m v%s — Pine parity. Symbol=%s TF=%s",
